...
describe('handleLifecycle', () => {
  ...
  it('keeps alive creatures as survivors and updates their stats', async () => {
    ...
    expect(result[0].stats.msLived).toBe(SIM_CONFIG.STATE_UPDATE_INTERVAL_MS);
  ...
  it('spawns offspring when creature reaches max energy', async () => {
    ...
    const parent = createCreature({
      id: 1,
      energy: SIM_CONFIG.CREATURE_MAX_ENERGY,
    });
    ...
    expect(survivor.energy).toBe(SIM_CONFIG.CREATURE_MAX_ENERGY - SIM_CONFIG.CREATURE_REPRODUCTION_ENERGY_COST);
